...
final class FutureImpl<T> implements Future<T> {
  ...
  public void onFailure(Consumer<Throwable> consumer) {
    ...
    onComplete(
        result -> {
          ...
          // should it be a Success it will be transformed into a Failure and forEach will do
          // nothing
  ...
  public <R> Future<R> flatMap(ThrowableFunction1<T, Future<R>> function) {
    ...
    onSuccess(
        value -> {
          ...
          // it might actually fail due to the provided function throws an exception, hence we wrap
          // it with a Try
          Try<Future<R>> mapped = Try(() -> function.apply(value));
          // add a recover function to the Try that fails this future with the error, else we
          // complete this Future with the mapped Future
    ...
    onSuccess(
        value -> {
          if (predicate.test(value)) {
            ...
            future.failure(
                new NoSuchElementException("The predicate failed on value [" + value + "]"));
          }
        });
  ...
  public <R> Future<R> transform(
    ...
    onFailure(
        t -> Try(() -> onFailure.apply(t)).recover(ex -> ex).forEach(ex -> future.failure(ex)));
    return future;
  }
   ...
   */
  @Override
  public FutureImpl<T> ready(long duration, TimeUnit timeUnit)
    ...
    // the actual result/response is of no interest as we anyways have access to it internally in
    // this class/instance.
    ...
    if (!latch.await(duration, timeUnit)) {
      throw new TimeoutException(
          "Timeout waiting [" + duration + "] [" + timeUnit + "] for Future to complete");
    }
