 ...
 * or {@link Promise#failure(Throwable) failure}.
 *
 * @see Executor
 * @author Peter Nerg
 * @since 1.2
 * @param <T> The type this executable is expected to work on
 */
@FunctionalInterface
public interface Executable<T> {
  ...
  /**
   * Execute the job. <br>
   * Any response needs to be reported via either {@link Promise#success(Object) Promise,success(T)}
   * or {@link Promise#failure(Throwable)}. <br>
   * When this method has finished it is imperative that the implementation has reported either
   * failure/success. If not the framework will automatically report the execution as a failure due
   * to a missing response.
   *
   * @param promise The promise to report the response to
   * @since 1.2
   */
